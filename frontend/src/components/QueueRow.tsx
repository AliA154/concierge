...
interface Props {
  ticket: Ticket; meta: Meta; selected: boolean; shake: boolean; collapsing: boolean;
...
function RowClass({ ticket, children, selected, shake, collapsing }: { ticket: Ticket; selected: boolean; shake: boolean; collapsing: boolean; children: ReactNode }) {
  const nowMs = useNow();
  const cls = ["row", ticket.is_vip ? "is-vip" : "", !ticket.is_vip && ticket.priority === "Critical" ? "is-crit" : "", selected ? "selected" : "", shake ? "shake" : "", collapsing ? "collapsing" : "", slaRowClass(ticket, nowMs)].filter(Boolean).join(" ");
  return <div className={cls} data-id={ticket.id} data-testid="queue-row">{children}</div>;
}

export const QueueRow = memo(function QueueRow({ ticket: t, meta, selected, shake, collapsing, onSelect, onOpen, onTake, onQuickState }: Props) {
  ...
  return (
    <div onClick={() => { onSelect(t.id); onOpen(t.id); }}>
      <RowClass ticket={t} selected={selected} shake={shake} collapsing={collapsing}>
